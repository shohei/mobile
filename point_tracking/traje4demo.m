...
[targetx2,targety2]=ginput(1);hold on;
plot([targetx1,targetx2],[targety1,targety2],'-');hold off;
...
axis equal; axis([-300,300,-100,500]);
grid; legend('Target','Point tracking method');
x=0;y=0;theta=0;
D=1;trajectory=[];theta=pi/2;

while(y<yt)
    steering = -atan(D*(6*ab(1)*y+2*ab(2))*...
        sin(theta)/((3*ab(1)*y^2+2*ab(2)*y)^2+1));
